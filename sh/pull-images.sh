...
# ============================================
# Mirror Configuration (from pull script)
# ============================================
...
# ============================================
# Pull image function with mirror support
# ============================================

pull_image_with_mirrors() {
    local image_path="$1"
    local full_image="docker.io/${image_path}"
    
    # Check if image exists locally
    if sudo ctr -n k8s.io images check "name==${full_image}" > /dev/null 2>&1; then
        return 0
    fi
    
    # Try mirrors
    for mirror in "${MIRRORS[@]}"; do
        local mirror_path="${mirror}/${image_path}"
        echo -ne "  ${CYAN}Trying ${mirror}... ${NC}"
        
        if sudo ctr -n k8s.io images pull "$mirror_path" > /dev/null 2>&1; then
            echo -e "${GREEN}✓${NC}"
            return 0
        else
            echo -e "${RED}✗${NC}"
    ...
    # Try direct Docker Hub as last resort
    echo -ne "  ${CYAN}Trying direct Docker Hub... ${NC}"
    if sudo ctr -n k8s.io images pull "$full_image" > /dev/null 2>&1; then
        echo -e "${GREEN}✓${NC}"
        return 0
    else
        echo -e "${RED}✗${NC}"
...
echo -e "${GRAY}  Pause Container:    ${NC}${PAUSE_VERSION} (target: ${TARGET_PAUSE})"
echo -e "${GRAY}  etcd:               ${NC}${ETCD_VERSION}"
echo -e "${GRAY}  CoreDNS:            ${NC}${COREDNS_VERSION}"
echo -e "${GRAY}  Mirrors:            ${NC}${MIRRORS[0]}, ${MIRRORS[1]}"
echo ""
...
run_with_spinner "Setting custom pause image" sudo sed -i "s|sandbox_image = .*|sandbox_image = \"registry.k8s.io/pause:${TARGET_PAUSE}\"|g" /etc/containerd/config.toml
run_with_spinner "Configuring CNI path" sudo sed -i 's|bin_dir = .*|bin_dir = "/opt/cni/bin"|g' /etc/containerd/config.toml

print_success "Pause image: registry.k8s.io/pause:${TARGET_PAUSE}"
...
# ============================================
# Step 4: Pull Custom Images (with mirror support)
# ============================================
...
declare -A K8S_IMAGE_MAP=(
    ["pause:${PAUSE_VERSION}"]="registry.k8s.io/pause:${TARGET_PAUSE}"
    ["kube-apiserver:${K8S_VERSION}"]="registry.k8s.io/kube-apiserver:v${K8S_VERSION}"
    ["kube-controller-manager:${K8S_VERSION}"]="registry.k8s.io/kube-controller-manager:v${K8S_VERSION}"
    ["kube-scheduler:${K8S_VERSION}"]="registry.k8s.io/kube-scheduler:v${K8S_VERSION}"
    ["kube-proxy:${K8S_VERSION}"]="registry.k8s.io/kube-proxy:v${K8S_VERSION}"
    ["etcd:${ETCD_VERSION}-riscv64"]="registry.k8s.io/etcd:${ETCD_VERSION}-0"
    ["coredns:${COREDNS_VERSION}"]="registry.k8s.io/coredns/coredns:v${COREDNS_VERSION}"
    ["flannel:${FLANNEL_VERSION}"]="docker.io/${DOCKERHUB_USER}/flannel:${FLANNEL_VERSION}"
)
...
echo ""
echo -e "${BOLD}${WHITE}Total images to check: ${TOTAL_IMAGES}${NC}"
echo -e "${DIM}${GRAY}Using mirrors: ${MIRRORS[*]}${NC}"
echo ""

for image in "${IMAGES[@]}"; do
    CURRENT=$((CURRENT + 1))
    FULL_IMAGE="${DOCKERHUB_USER}/${image}"
    K8S_TARGET="${K8S_IMAGE_MAP[$image]}"
    
    echo -e "${BOLD}${WHITE}[${CURRENT}/${TOTAL_IMAGES}]${NC} ${GRAY}${image}${NC}"
    
    # Check if image exists locally (check both source and target)
    if sudo ctr -n k8s.io images check "name==docker.io/${FULL_IMAGE}" > /dev/null 2>&1 || \
       sudo ctr -n k8s.io images check "name==${K8S_TARGET}" > /dev/null 2>&1; then
        echo -e "  ${GREEN}✓ Already exists, skipping pull${NC}"
        SKIPPED_COUNT=$((SKIPPED_COUNT + 1))
        echo ""
        continue
    fi
    
    # Pull image with mirror support
    echo -e "  ${CYAN}⬇ Pulling with mirrors...${NC}"
    if pull_image_with_mirrors "${FULL_IMAGE}"; then
        echo -e "  ${GREEN}✓ Pull successful${NC}"
        PULLED_COUNT=$((PULLED_COUNT + 1))
        
        # Tag image immediately
        if [[ "$image" != "flannel:${FLANNEL_VERSION}" ]]; then
            ctr_retag "docker.io/${FULL_IMAGE}" "${K8S_TARGET}"
        fi
    else
...
# Special handling: also tag coredns for v1.13.1 (kubeadm default)
print_info "Tagging CoreDNS for kubeadm compatibility..."
if sudo ctr -n k8s.io images check "name==registry.k8s.io/coredns/coredns:v${COREDNS_VERSION}" > /dev/null 2>&1; then
    sudo ctr -n k8s.io images tag \
        "registry.k8s.io/coredns/coredns:v${COREDNS_VERSION}" \
        "registry.k8s.io/coredns/coredns:v1.13.1" 2>/dev/null || true
    print_success "CoreDNS v1.13.1 tag created for kubeadm compatibility"
fi
...
echo -e "  Pulled:   ${CYAN}${PULLED_COUNT}${NC}"
...
# Build/Download Flannel CNI plugin
print_info "Installing Flannel CNI plugin..."
if [ ! -f /opt/cni/bin/flannel ]; then
    (
        # Try downloading pre-built binary first
        wget -q -O /opt/cni/bin/flannel \
            https://github.com/flannel-io/cni-plugin/releases/download/v1.4.1-flannel1/flannel-riscv64 2>/dev/null && \
    ...
    if [ ! -f /opt/cni/bin/flannel ]; then
        ...
        (
            cd /tmp
            rm -rf cni-plugin
            git clone --depth=1 https://github.com/flannel-io/cni-plugin.git 2>/dev/null
            cd cni-plugin
        ...
        echo -e "${GREEN}  ✓${NC} Flannel CNI plugin downloaded successfully"
    fi
else
    print_success "Flannel CNI plugin already installed"
fi
...
if [ -f /opt/cni/bin/flannel ]; then
    ...
    print_error "Flannel CNI plugin installation failed"
    exit 1
fi
...
(
    sudo kubeadm init --config /tmp/kubeadm-config.yaml > /tmp/kubeadm_init.log 2>&1
